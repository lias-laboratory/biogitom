...
# Import PyTorch Geometric's graph layers:
...
src_ent = "ncit"        # Source ontology
tgt_ent = "doid"        # Target ontology
...
# ======================================================
#  GT Architecture (Graph Transformer à la place de GCN)
# ======================================================

class GT_mod(nn.Module):
    """
    Multi-layer Graph Transformer basé sur TransformerConv.
    La dimension de sortie reste égale à la dimension d'entrée (dim_h)
    pour rester compatible avec le GatedCombination.
    """
    def __init__(self, dim_h, num_layers=2, num_linear_layers=1, dropout=0.0, heads=1):
        super().__init__()
        self.num_layers = num_layers
        self.num_linear_layers = num_linear_layers
        self.dropout = dropout

        # Pré-traitement linéaire (comme avant)
        self.linears = nn.ModuleList()
        for _ in range(num_linear_layers):
            self.linears.append(Linear(dim_h, dim_h))
            self.linears.append(PReLU(num_parameters=dim_h))

        # Couches Graph Transformer (TransformerConv)
        ...
        for _ in range(num_layers):
            conv = TransformerConv(
                in_channels=dim_h,
                out_channels=dim_h,   # dim sortie = dim_h (heads=1)
                heads=heads,
                dropout=dropout,
                edge_dim=None,
                beta=False
    ...
    def forward(self, x, edge_index):
        ...
        # 2) Propagation Graph Transformer
...
def contrastive_loss(source_embeddings, target_embeddings, labels, margin=1.0):
    """
    Computes contrastive loss for matching / similarity learning.
    """
...
def save_gated_embeddings(gated_model, embeddings_src, x_src, embeddings_tgt, x_tgt,
    ...
    """
    Compute and save final entity embeddings (gated) for both source and target ontologies.
    """
    import pandas as pd
    import torch
    import time
...
def filter_ignored_class(src_emb_path, tgt_emb_path, src_onto, tgt_onto, Emb_final_src_cl, Emb_final_tgt_cl):
    """
    Filters embeddings by removing concepts considered 'ignored classes' in both ontologies.
    """
...
def evaluate_topk(topk_file, train_file, test_file, k=1, threshold=0.0):
    ...
    after filtering and 1-1 constraint.
    """
...
print("Graph Transformer (GT) Training...")
...
# Initialize the GT_mod model
GT_model = GT_mod(dim_h=x_tgt.shape[1], num_layers=2, num_linear_layers=1, dropout=0.0, heads=1)
...
trained_model_gnn.eval()

with torch.no_grad():
    embeddings_tgt = trained_model_gnn(x_tgt, edge_tgt)
    embeddings_src = trained_model_gnn(x_src, edge_src)
...
output_file_eval, metrics, correct = evaluate_predictions(
