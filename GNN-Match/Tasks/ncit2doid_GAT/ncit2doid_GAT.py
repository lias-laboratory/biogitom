...
# Import PyTorch Geometric's GAT layer:
...
# ======================================================
#  GAT Architecture (Graph Attention Network)
# ======================================================

class GAT_mod(nn.Module):
    """
    Multi-layer Graph Attention Network basé sur GATConv.
    La dimension de sortie reste égale à la dimension d'entrée (dim_h)
    pour rester compatible avec le GatedCombination.
    """
    def __init__(self, dim_h, num_layers=2, num_linear_layers=1, dropout=0.0, heads=4):
        super().__init__()
        ...
        # On met concat=False pour garder la dimension dim_h (out_channels = dim_h)
        for _ in range(num_layers):
            conv = GATConv(
                in_channels=dim_h,
                out_channels=dim_h,   # comme concat=False, sortie = dim_h
                heads=heads,
                concat=False,
                dropout=dropout
            )
...
class GatedCombination(nn.Module):
    ...
    def faiss_l2(self, a, b):
        ...
        index.add(b_np)
        distances, _ = index.search(a_np, 1)
...
# Initialize the GAT_mod model
GAT_model = GAT_mod(dim_h=x_tgt.shape[1], num_layers=2, num_linear_layers=1, dropout=0.0, heads=4)
