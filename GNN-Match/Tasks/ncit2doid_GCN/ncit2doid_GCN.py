...
src_ent = "ncit"        # SNOMED CT pharmaceutical concepts
tgt_ent = "doid"        # NCIT pharmaceutical concepts

# Define the matching task name
task = "ncit2doid"                  # Used to name intermediate and output files
...
# ======================================================
#  GCN Architecture (remplace RGIT / GIT par un GCN simple)
# ======================================================

class GCN_mod(nn.Module):
    """
    Multi-layer GCN with optional linear pre-processing.
    La dimension de sortie est la même que la dimension d'entrée
    pour rester compatible avec le GatedCombination.
    """
    def __init__(self, dim_h, num_layers=2, num_linear_layers=1, dropout=0.0):
        super().__init__()
        self.num_layers = num_layers
        self.num_linear_layers = num_linear_layers
        self.dropout = dropout

        # Pré-traitement linéaire (comme dans RGIT_mod)
...
# Initialize the GCN_mod model with the dimensionality of the target embeddings
# dim_h = x_tgt.shape[1]
GCN_model = GCN_mod(dim_h=x_tgt.shape[1], num_layers=2, num_linear_layers=1, dropout=0.0)
...
trained_model = train_model_gnn(
    model=GCN_model,                # The GNN model to be trained (initialized earlier)
...
# Move the trained GCN model to the device (GPU or CPU)
...
with torch.no_grad():  # Disable gradient computation (inference mode)
    embeddings_tgt = trained_model(x_tgt, edge_tgt)  # Get updated embeddings for the target graph
    embeddings_src = trained_model(x_src, edge_src)  # Get updated embeddings for the source graph
