...
# (On n'a plus besoin de MessagePassing / Linear de PyG pour RGIT, donc on ne les importe plus)
...
# ======================================================
#  🧠 GCN Architecture (multi-layer GCN)
# ======================================================

class GCN_mod(nn.Module):
    """
    Multi-layer GCN with optional linear pre-processing.
    La dimension de sortie = dimension d'entrée (compatible GatedCombination).
    """
    def __init__(self, dim_h, num_layers=2, num_linear_layers=1, dropout=0.0):
        ...
        # Pré-traitement linéaire (comme dans les versions précédentes)
...
def contrastive_loss(source_embeddings, target_embeddings, labels, margin=1.0):
    """
    Computes the contrastive loss for matching.
    """
...
def save_gated_embeddings(gated_model, embeddings_src, x_src, embeddings_tgt, x_tgt,
    ...
    for both source and target ontologies.
    """
    import time
...
def select_best_candidates_per_src_with_margin(df, score_margin=0.01):
    ...
    within (1 - score_margin) of the best score.
    """
    selected_rows = []
...
GCN_model = GCN_mod(x_tgt.shape[1], num_layers=1, num_linear_layers=1, dropout=0.0)

# Reading the training pairs
...
trained_gcn_model = train_model_gnn(
    model=GCN_model,
    x_src=x_src,
    edge_src=edge_src,
    x_tgt=x_tgt,
...
with torch.no_grad():
    embeddings_tgt = trained_gcn_model(x_tgt, edge_tgt)
    embeddings_src = trained_gcn_model(x_src, edge_src)
...
def train_gated_combination_model(X1_t, X2_t, X3_t, X4_t, tensor_score_o,
    ...
    """
    Trains the GatedCombination model with training and validation data.
    """
...
trained_model = train_gated_combination_model(
    X1_train, X2_train, X3_train, X4_train, tensor_score_train,
    X1_val, X2_val, X3_val, X4_val, tensor_score_val,
    epochs=100, batch_size=32, learning_rate=0.001, weight_decay=1e-4
)
...
results_mrr = ranking_eval(formatted_predictions_path, Ks=[1, 5, 10])
