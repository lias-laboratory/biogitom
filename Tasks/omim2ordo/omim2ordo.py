...
# Define the source and target ontology identifiers
src_ent = "omim"        # SNOMED CT pharmaceutical concepts
tgt_ent = "ordo"          # NCIT pharmaceutical concepts

# Define the matching task name
task = "omim2ordo"                  # Used to name intermediate and output files
...
class GatedCombination(nn.Module):
    def __init__(self, input_dim):
        ...
        # Linear layers to compute gating values for the two input pairs
        self.gate_A_fc = nn.Linear(input_dim, input_dim)
        self.gate_B_fc = nn.Linear(input_dim, input_dim)
        # Final classification layer to convert similarity score into a probability
        self.fc = nn.Linear(1, 1)

    def diem_distance(self, a, b, eps=1e-8):
        """
        Compute the DIEM similarity score based on the normalized squared L2 distance.
        ...
        Returns:
            Tensor of shape [batch_size] containing DIEM scores
        """
        diff = a - b
        euclidean_sq = torch.sum(diff ** 2, dim=1)  # Squared Euclidean distance
        norm_a = torch.norm(a, dim=1)               # Norm of vector a
        norm_b = torch.norm(b, dim=1)               # Norm of vector b
        denom = norm_a * norm_b + eps               # Product of norms (add eps to avoid division by zero)
        return euclidean_sq / denom

    def forward(self, x1, x2, x3, x4, return_embeddings=False):
        """
        Forward pass of the GatedCombination model.

        Args:
            x1, x2: Tensors representing two versions of the source entity embeddings
            x3, x4: Tensors representing two versions of the target entity embeddings
            return_embeddings: If True, return the gated embeddings a and b instead of the classification score

        Returns:
            If return_embeddings is True: (a, b)
            Else: classification probability (Tensor of shape [batch_size, 1])
        """
        # Gating mechanism for the source entity
        gate_values1 = torch.sigmoid(self.gate_A_fc(x1))
        a = x1 * gate_values1 + x2 * (1 - gate_values1)

        # Gating mechanism for the target entity
        gate_values2 = torch.sigmoid(self.gate_B_fc(x3))
        b = x3 * gate_values2 + x4 * (1 - gate_values2)

        # If embeddings are requested, return them directly
        if return_embeddings:
            return a, b

        # Compute DIEM similarity between the gated embeddings
        diem_score = self.diem_distance(a, b)

        # Classification step: map similarity score to probability with sigmoid
...
def evaluate_predictions(pred_file, train_file, test_file,  k=1, threshold=0.0, use_null_ref=True):
    """
    ...
    output_file1 = pred_file.replace(".tsv", "_filtered.tsv")
