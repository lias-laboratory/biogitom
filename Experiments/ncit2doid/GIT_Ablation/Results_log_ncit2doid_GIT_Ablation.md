...
```
    ...
    <ipython-input-37-eeee9defe2d6> in <cell line: 0>()
    ...
    <ipython-input-34-0901fe295449> in train_model_gnn(model, x_src, edge_src, x_tgt, edge_tgt, tensor_term1, tensor_term2, tensor_score, learning_rate, weight_decay_value, num_epochs, print_interval)
         62 
...
output_file_src = f"{data_dir}/{src_ent}_final_O_GIT_embeddings.tsv"
output_file_tgt = f"{data_dir}/{tgt_ent}_final_O_GIT_embeddings.tsv"
...
```
    ...
    - Source: /content/gdrive/My Drive/BioGITOM-VLDB//ncit2doid/Data/ncit_final_O_GIT_embeddings.tsv
    - Target: /content/gdrive/My Drive/BioGITOM-VLDB//ncit2doid/Data/doid_final_O_GIT_embeddings.tsv
    ⏱️ Execution time: 22.12 seconds
...
src_file, tgt_file = filter_ignored_class(
    src_emb_path=output_file_src,
    tgt_emb_path=output_file_tgt,
    src_onto=src_onto,
    tgt_onto=tgt_onto
...
```
    ...
    📁 Cleaned source file saved to: /content/gdrive/My Drive/BioGITOM-VLDB//ncit2doid/Data/ncit_final_O_GIT_embeddings_cleaned.tsv
    📁 Cleaned target file saved to: /content/gdrive/My Drive/BioGITOM-VLDB//ncit2doid/Data/doid_final_O_GIT_embeddings_cleaned.tsv
...
topk_faiss_l2(
    src_emb_path=f"{data_dir}/{src_ent}_O_GIT_embeddings_cleaned.tsv",
    tgt_emb_path=f"{data_dir}/{tgt_ent}_O_GIT_embeddings_cleaned.tsv",
    top_k=10,
    output_file=f"{results_dir}/{task}_top_10_mappings_O_GIT.tsv"
)
```

    🔹 Using L2 (Euclidean) distance with FAISS
    Top-10 FAISS similarity results saved to: /content/gdrive/My Drive/BioGITOM-VLDB//ncit2doid/Results/ncit2doid_top_10_mappings_O_GIT.tsv
    ⏱️ Execution time: 3.87 seconds
...
output_file, metrics, correct = evaluate_predictions(
    topk_file=f"{results_dir}/{task}_top_10_mappings_O_GIT.tsv",
...
```
       ...
       ➤ Mappings file:   /content/gdrive/My Drive/BioGITOM-VLDB//ncit2doid/Results/ncit2doid_top_10_mappings_O_GIT_predictions.tsv
    
    🎯 Evaluation Summary:
       - Correct mappings:     2932
...
topk_faiss_l2(
    ...
    tgt_emb_path=f"{data_dir}/{tgt_ent}_O_GIT_embeddings_cleaned.tsv",
    top_k=1,
    output_file=f"{results_dir}/{task}_top_1_mappings_O_GIT.tsv"
)
```

    🔹 Using L2 (Euclidean) distance with FAISS
    Top-1 FAISS similarity results saved to: /content/gdrive/My Drive/BioGITOM-VLDB//ncit2doid/Results/ncit2doid_top_1_mappings_O_GIT.tsv
...
results = evaluate_topk(
    topk_file=f"{results_dir}/{task}_top_1_mappings_O_GIT.tsv",
