...
```
    ...
    [2K     [90m━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━[0m [32m62.0/62.0 kB[0m [31m763.4 kB/s[0m eta [36m0:00:00[0m
    ...
    [2K   [90m━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━[0m [32m766.7/766.7 MB[0m [31m456.3 kB/s[0m eta [36m0:00:00[0m
    ...
    [2K   [90m━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━[0m [32m56.3/56.3 MB[0m [31m1.2 MB/s[0m eta [36m0:00:00[0m
    ...
    [2K   [90m━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━[0m [32m150.1/150.1 MB[0m [31m7.6 MB/s[0m eta [36m0:00:00[0m
    ...
    [2K   [90m━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━[0m [32m6.2/6.2 MB[0m [31m115.2 MB/s[0m eta [36m0:00:00[0m
    [?25hDownloading triton-3.2.0-cp311-cp311-manylinux_2_17_x86_64.manylinux2014_x86_64.whl (253.2 MB)
    [2K   [90m━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━[0m [32m253.2/253.2 MB[0m [31m4.3 MB/s[0m eta [36m0:00:00[0m
    [?25hDownloading pillow-11.2.1-cp311-cp311-manylinux_2_28_x86_64.whl (4.6 MB)
    [2K   [90m━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━[0m [32m4.6/4.6 MB[0m [31m110.4 MB/s[0m eta [36m0:00:00[0m
    ...
    [2K   [90m━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━[0m [32m16.8/16.8 MB[0m [31m116.2 MB/s[0m eta [36m0:00:00[0m
...
```
    ...
    <ipython-input-39-eeee9defe2d6> in <cell line: 0>()
          1 # Train the GNN model using the provided source and target graph embeddings, edges, and training data
    ----> 2 trained_model = train_model_gnn(
          3     model=GIT_model,                # The GNN model to be trained (initialized earlier)
          4     x_src=x_src,                    # Source node embeddings (tensor for source graph)
          5     edge_src=edge_src,              # Source graph edges (undirected edge index for source graph)


    <ipython-input-36-0901fe295449> in train_model_gnn(model, x_src, edge_src, x_tgt, edge_tgt, tensor_term1, tensor_term2, tensor_score, learning_rate, weight_decay_value, num_epochs, print_interval)
         62 
         63         # Backward pass: Compute gradients
    ---> 64         loss.backward()
         65 
         66         # Update the model's parameters
...
```
    ...
    Epoch [1/100] Training Loss: 0.0503, F1 Score: 0.0000 | Validation Loss: 0.0501, F1 Score: 0.0000
    Epoch [2/100] Training Loss: 0.0459, F1 Score: 0.0000 | Validation Loss: 0.0456, F1 Score: 0.0000
    Epoch [3/100] Training Loss: 0.0417, F1 Score: 0.0519 | Validation Loss: 0.0413, F1 Score: 0.0970
    ...
    <ipython-input-44-bba083b58584> in <cell line: 0>()
          1 # Train the GatedCombination model using training and validation data
    ----> 2 trained_model = train_gated_combination_model(
          3     X1_train,          # Updated source embeddings (after applying the GNN model)
          4     X2_train,          # Original source embeddings (before applying the GNN model)
          5     X3_train,          # Updated target embeddings (after applying the GNN model)


    <ipython-input-43-0f7a8823eec8> in train_gated_combination_model(X1_t, X2_t, X3_t, X4_t, tensor_score_o, X1_val, X2_val, X3_val, X4_val, tensor_score_val, epochs, batch_size, learning_rate, weight_decay)
         47             loss = F.binary_cross_entropy(outputs, batch_y.unsqueeze(1).float())
         48             loss.backward()
    ---> 49             optimizer.step()
    ...
    /usr/local/lib/python3.11/dist-packages/torch/optim/adam.py in _single_tensor_adam(params, grads, exp_avgs, exp_avg_sqs, max_exp_avg_sqs, state_steps, grad_scale, found_inf, amsgrad, has_complex, beta1, beta2, lr, weight_decay, eps, maximize, capturable, differentiable)
        474                 denom = (max_exp_avg_sqs[i].sqrt() / bias_correction2_sqrt).add_(eps)
        475             else:
    --> 476                 denom = (exp_avg_sq.sqrt() / bias_correction2_sqrt).add_(eps)
        477 
        478             param.addcdiv_(exp_avg, denom, value=-step_size)
...
output_file_src = f"{data_dir}/{src_ent}_final_embeddings_O_GIT.tsv"
output_file_tgt = f"{data_dir}/{tgt_ent}_final_embeddings_O_GIT.tsv"

# Save the final gated embeddings for all concepts in source and target ontologies
save_semantic_only_embeddings(
...
```
    ...
    ✅ Semantic-only embeddings saved:
    - Source: /content/gdrive/My Drive/BioGITOM-VLDB//neoplas/Data/snomed.neoplas_final_embeddings_O_GIT.tsv
    - Target: /content/gdrive/My Drive/BioGITOM-VLDB//neoplas/Data/ncit.neoplas_final_embeddings_O_GIT.tsv
    ⏱️ Execution time: 30.88 seconds
...
src_file, tgt_file = filter_ignored_class(
    src_emb_path=f"{data_dir}/{src_ent}_final_embeddings_O_GIT.tsv",
    tgt_emb_path=f"{data_dir}/{tgt_ent}_final_embeddings_O_GIT.tsv",
    src_onto=src_onto,
    tgt_onto=tgt_onto
...
```
    ...
    📁 Cleaned source file saved to: /content/gdrive/My Drive/BioGITOM-VLDB//neoplas/Data/snomed.neoplas_final_embeddings_O_GIT_cleaned.tsv
    📁 Cleaned target file saved to: /content/gdrive/My Drive/BioGITOM-VLDB//neoplas/Data/ncit.neoplas_final_embeddings_O_GIT_cleaned.tsv
...
topk_faiss_l2(
    src_emb_path=f"{data_dir}/{src_ent}_final_embeddings_O_GIT_cleaned.tsv",
    tgt_emb_path=f"{data_dir}/{tgt_ent}_final_embeddings_O_GIT_cleaned.tsv",
    top_k=10,
    output_file=f"{results_dir}/{task}_top_10_mappings_faiss_l2_O_GIT.tsv"
)
```

    🔹 Using L2 (Euclidean) distance with FAISS
    Top-10 FAISS similarity results saved to: /content/gdrive/My Drive/BioGITOM-VLDB//neoplas/Results/neoplas_top_10_mappings_faiss_l2_O_GIT.tsv
    ⏱️ Execution time: 6.31 seconds
...
output_file, metrics, correct = evaluate_predictions(
    pred_file=f"{results_dir}/{task}_top_10_mappings_faiss_l2_O_GIT.tsv",
...
```
    ...
    ✅ After removing train-only URIs: 95442 rows
    ✅ After keeping only test SrcEntities: 22586 rows
    ✅ After applying threshold ≥ 0.0: 22586 rows
    📁 Filtered predictions saved: /content/gdrive/My Drive/BioGITOM-VLDB//neoplas/Results/neoplas_top_10_mappings_faiss_l2_O_GIT_filtered.tsv
    🏆 Selected candidates within 99.2% of best score per SrcEntity: 2525 rows
    📁 Filtered Top-1 file saved: /content/gdrive/My Drive/BioGITOM-VLDB//neoplas/Results/neoplas_top_10_mappings_faiss_l2_O_GIT_filtered_top1_th0.0.tsv
    🎯 Correct mappings (Top-1): 1788
    📊 Evaluation (P / R / F1): {'P': 0.708, 'R': 0.671, 'F1': 0.689}
